...
<?php
    ...
    if ($aiAlert !== null) {
        ...
        $aiResult = $webhook->getAIDetectionResult();
        if ($aiResult !== null && $aiResult->summary !== null) {
            echo "Model " . $aiResult->modelVersion . ": AI " . $aiResult->summary->ai . ", human " . $aiResult->summary->human . "\n";
            foreach ($aiResult->results as $result) {
                echo "Segment classification: " . $result->classification . "\n"; // 1 = human, 2 = AI
            }
        }
    }
```
A null alert does not by itself prove that AI detection ran: check the scan's `aiGeneratedText.detect` setting and the other alert codes in `CopyleaksAlertCodes` (for example `AI_DETECTION_FAILED`). The raw JSON string stays available in `$aiAlert->additionalData`.
##
